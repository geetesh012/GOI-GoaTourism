...
export function startSmoothScroll() {
  ...
  lenis = new Lenis({
    duration: reduceMotion ? 0 : 1.15, // higher = slower/softer glide
    easing: (t) => 1 - Math.pow(1 - t, 4), // quart-out, matches EASE.entrance elsewhere
    smoothWheel: !reduceMotion,
    syncTouch: false, // don't fight native touch scrolling on mobile
  })
...
export function scrollToY(y, opts = {}) {
  if (lenis) {
    lenis.scrollTo(y, { duration: 1.4, easing: (t) => 1 - Math.pow(1 - t, 4), ...opts })
  } else {
...
export function scrollToEl(el, opts = {}) {
  if (!el) return
  if (lenis) {
    lenis.scrollTo(el, { duration: 1.2, easing: (t) => 1 - Math.pow(1 - t, 4), ...opts })
  } else {
